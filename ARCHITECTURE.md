...
**Status:** A1–A3, B1, and C3 are **DONE** (Session 11, approved & implemented — see the ✅ markers
below). Sections 4 (backend stubs) and C1–C2 remain proposals awaiting a go-ahead. The Session-10
test-folder move is also done.

**Test count after C3:** 13 files / **103 tests** (up from 6 / 54). Newly covered: the pagination helper,
all three zod schemas (dashboard/chart/filters), both service facades' full CRUD + widget ops +
ownership authorization (not just `getChartsByIds`), explicit read/write wrapper boundary tests,
`useDebouncedValue`, and one query+mutation hook integration test (QueryClient wrapper + mocked network).
Still untested by design: the RGL-rendered grid
interaction (jsdom has no layout box — see B2; covered by Playwright), and the RHF form *components*
themselves (their validation logic is covered via the schema tests).

**Context:** M2 frontend, 11 sessions of incremental build + QA/review, currently verified-correct
(tsc/eslint/build clean, 103 tests passing, Docker healthy on :3100 as of the last container pass). The bar for change is high — this
...
## 2. Quick, safe cleanups — ✅ DONE (Session 11)

| # | Finding | Evidence | Change made | Risk |
|---|---------|----------|-------------|------|
...
| A2 ✅ | **`useChartsByIds` title-skeleton flash on add/remove.** Adding/removing a widget changes `chartIds` → changes the query key → `isPending` flips true → `chartNamePending` true for **every** widget → all titles flash to skeleton, even unchanged ones. | `hooks/use-charts.ts` (`byIds(sortedIds)` key), `dashboard-view.tsx:206` | Add `placeholderData: (prev) => prev` to `useChartsByIds` so prior names stay visible, and compute `chartNamePending` per widget using `isFetching`/`isPlaceholderData` so a newly added widget doesn't briefly render "Unavailable chart" while its id resolves. | Very low — same pattern already used in `use-dashboard-results`. |
...
1. **Done:** A1–A3 + B1 + C3, verified against the full local gate set.
2. **Next, if wanted:** C1 (route error/not-found/loading files) — additive robustness.
3. **Then, product-gated:** C2 (filter-in-URL).
4. **Backend-gated (Section 4):** as the real `cia-backend` lands.

*Remaining proposals should still be implemented one at a time, gated on tsc/eslint/build/test + Docker,
same discipline as prior sessions.*
